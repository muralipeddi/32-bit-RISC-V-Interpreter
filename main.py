...
class DataMem(object):
    ...
    def writeDataMem(self, Address, WriteData):
        print(f'{Address=}, {WriteData=}')
        # Split the 32-bit integer into four bytes and store them in little-endian order (least significant byte first)
        # for i in range(4):
        #     byte = (WriteData >> (8 * i)) & 0xFF  # Extract one byte
        #     self.DMem[Address + i] = format(byte, '08b')  # Convert to binary string and store
        mask = 255
        data = []
        for i in range(4):
...
class State(object):
    def __init__(self):
        self.IF = {"nop": False, "PC": 0, "taken": bool(False)}
...
class SingleStageCore(Core):
 ...
 #Your implementation
    ...
    def step(self):
        ...
        elif self.state.IF["PC"] < (len(self.ext_imem.IMem) - 4):
    ...
    def decode(self, instruction):
        ...
        if opcode == '0110011':  # R-type
        ...
        elif opcode == '0000011':  # LW instruction
                imm = int(instruction[0:12], 2)  # Immediate value
                rs1 = int(instruction[12:17], 2)  # Source register
                rd = int(instruction[20:25], 2)  # Destination register
                ...
                imm5 = int(instruction[0:7], 2) & 31 # imm[4:0]
                imm7 = int(instruction[20:25], 2)  # imm[11:5]
                ...
                if funct3 == '000':  # BEQ
                    alu_op = 'BEQ'
                    if rs1_val == rs2_val:
                        self.nextState.IF["PC"] = self.state.IF["PC"] + sign_extension(imm, 12)
                ...
                self.nextState.IF["PC"] = self.state.IF["PC"] + sign_extension(imm, 20)
                self.state.IF["taken"] = True
            
            
        elif opcode == '1111111':  # Special case for HALT
    ...
    def execute(self):
            ...
            if alu_op == 'ADD':
                ...
                self.state.MEM["ALUresult"] = result
                ...
                result = current_pc + 4
                self.state.MEM["ALUresult"] = result
                self.state.MEM["jal_target"] = current_pc + imm_value

            elif alu_op == "LW":
                rs1_val = self.myRF.readRF(self.state.EX["Rs"])
                data = self.ext_dmem.readInstr(rs1_val + sign_extension(self.state.EX["Imm"], 11))
                data_hex = int(data, 16)
                self.myRF.writeRF(self.state.EX["Wrt_reg_addr"], data_hex)

            elif alu_op == "SW":
                rs2_val = self.myRF.readRF(self.state.EX["Rt"])
                address = self.state.EX["Rs"] + sign_extension(self.state.EX["Imm"], 11)
                address = address & ((1 << 32) - 1)
                self.ext_dmem.writeDataMem(address, rs2_val)
    ...
    #         address = self.state.MEM["ALUresult"]  # Get the address
...
def get_metrics(filePath ,ssCore: SingleStageCore):
    singleCore = [
        "Performance of Single Stage: ",
        f"#Cycles -> {ssCore.cycle + 1}",
        f"#Instructions -> {ssCore.instruction_count}",
        f"CPI -> {(ssCore.cycle + 1 )/ ssCore.instruction_count}",
        f'IPC -> {ssCore.instruction_count / (ssCore.cycle + 1)}'
    ]

    with open(filePath + os.sep + "PerformanceMetrics_Result", "w") as f:
        f.write("\n".join(singleCore))
